...
/*   Updated: 2024/05/09 14:06:31 by migupere         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
    while (cmd->tokens_argv[++(*arg_index)]->value)
    {
        if (cmd->tokens_argv[*arg_index]->value[0] != '-')
            break;
        if (!ft_contains_only(&cmd->tokens_argv[*arg_index]->value[1], "n"))
            break;
        return 1;
    }
    return 0;
