...
/*   Updated: 2024/07/04 15:19:09 by pevieira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	setup_pipe_run_writer(t_shell *shell, t_ast_list *cmd, int fd[2])
{
	check_error_and_exit(pipe(fd), "pipe error", FORK_FAILURE_EXIT_CODE);
	shell->pid = check_fork();
	if (shell->pid < 0)
...
static void	run_reader_finalize_pipe(t_shell *shell, t_ast_list *cmd, int fd[2])
{
	if (shell->status == CONTINUE)
		shell->pid = check_fork();
	if (shell->pid == 0)
...
static void	run_pipe(t_shell *shell, t_ast_list *cmd)
{
	...
	setup_pipe_run_writer(shell, cmd, fd);
	run_reader_finalize_pipe(shell, cmd, fd);
}
