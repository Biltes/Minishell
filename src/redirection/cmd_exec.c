...
/*   Updated: 2024/05/17 15:56:39 by pevieira         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	run_pipe(t_shell *shell, t_ast_list *cmd)
{
    ...
    {
        check_error_and_exit(dup2(fd[1], STDOUT_FILENO), "dup2 error", FORK_FAILURE_EXIT_CODE);
	...
	if (shell->pid == 0)
	{
		check_error_and_exit(dup2(fd[0], STDIN_FILENO), "dup2 error", FORK_FAILURE_EXIT_CODE);
		close_fds_and_signals_set(fd, SIGIGNORE);
